...
function ProgressBar({ label, icon, value, max, color = 'blue' }) {
  ...
  const barColors = { blue: 'bg-orange-600', purple: 'bg-orange-400' };
  const barColor = pct > 90 ? 'bg-red-500' : pct > 70 ? 'bg-yellow-500' : barColors[color];

  return (
    <div className="bg-gray-800/50 border border-gray-700/30 p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.1em] text-gray-400">
          {icon}
          {label}
        </span>
        <span className="text-xs text-gray-300 font-mono">{fmt(value)} / {fmt(max)}</span>
      </div>
      <div className="w-full bg-gray-700/50 h-2 overflow-hidden">
        <div className={`${barColor} h-full transition-all duration-700`} style={{ width: `${clampedPct}%` }} />
      </div>
      ...
      <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {isRunning ? (
              <>
                ...
                <ActionBtn color="blue" onClick={() => window.open(`/vnc/${routeNode(vm)}/${vm.vmid}${vm.name ? `?name=${encodeURIComponent(vm.name)}` : ''}`, '_blank', 'noopener')} icon={
                ...
                <ActionBtn color="blue" onClick={() => window.open(`/ssh/${routeNode(vm)}/${vm.vmid}${vm.name ? `?name=${encodeURIComponent(vm.name)}` : ''}`, '_blank', 'noopener')} icon={
            ...
            <ActionBtn color="red" onClick={() => setDeleteModal(true)} icon={
        ...
        {isRunning && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="aaris-display text-sm text-gray-100">Performance</h2>
            ...
            {rrdLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <MiniChart data={cpuData}       color="#ff5a1f" label="CPU"         formatValue={v => v != null ? `${v.toFixed(1)}%` : '—'} />
                <MiniChart data={memData}        color="#ff8a55" label="Memory"      formatValue={v => v != null ? `${v.toFixed(1)}%` : '—'} />
                <MiniChart data={netInData}      color="#3fd97f" label="Network In"  formatValue={fmtRate} />
                <MiniChart data={netOutData}     color="#ffb224" label="Network Out" formatValue={fmtRate} />
                <MiniChart data={diskReadData}   color="#8b939e" label="Disk Read"   formatValue={fmtRate} />
                <MiniChart data={diskWriteData}  color="#e9ecef" label="Disk Write"  formatValue={fmtRate} />
              </div>
            )}
          </div>
        )}
        ...
        <div className="space-y-3">
          <h2 className="aaris-display text-sm text-gray-100">SSH Configuration</h2>
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
...
function CloudInitCredentialsModal({ vm, node, vmid, ciuser, onClose, onApplied }) {
  ...
  const [error, setError] = useState('');
  const [result, setResult] = useState(null); // { what, rebooted, running } once applied
  const pwId = useId();
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Show the outcome inline (incl. the important reboot notice) rather than
      // closing immediately — reboot only applies the change on next boot.
      setResult({ what, rebooted: !!r.data?.rebooted, running: vm?.status === 'running' });
      onApplied?.();
    } catch (e) {
  ...
  // Applied — show the outcome and (crucially) the reboot notice.
  if (result) {
    return (
      <Modal title="Reset cloud-init credentials" onClose={onClose} size="md">
        <div className="p-5 space-y-4">
          {result.rebooted ? (
            <div className="flex items-start gap-3 bg-green-900/20 border border-green-800/50 rounded-xl p-4">
              <svg className="w-5 h-5 text-green-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <div className="text-xs text-green-300/90 space-y-1">
                <p className="font-medium text-green-300">Credentials updated</p>
                <p>Cloud-init {result.what} updated. The VM is {result.running ? 'rebooting' : 'starting'} to apply the new credentials.</p>
              </div>
            </div>
          ) : (
            <div className="flex items-start gap-3 bg-amber-900/20 border border-amber-800/50 rounded-xl p-4">
              ...
              <div className="text-xs text-amber-200/90 space-y-1">
                <p className="font-medium text-amber-300">Reboot required</p>
                <p>Cloud-init {result.what} updated. Reboot the VM to apply the new credentials.</p>
              </div>
            </div>
          )}
          <div className="flex items-center justify-end">
            <button onClick={onClose} className="text-xs px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium transition-colors">Done</button>
          </div>
        </div>
      </Modal>
    );
...
function DeleteVMModal({ vm, node, onClose }) {
  ...
  const [error, setError] = useState('');
  const expected = vm.name || `VM ${vm.vmid}`;
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.error || 'Failed to delete VM');
      setDeleting(false);
    }
  };

  return (
    <Modal title="Delete VM" onClose={deleting ? () => {} : onClose} size="sm">
      <div className="p-5 space-y-4">
        <div className="flex items-start gap-3 bg-red-900/20 border border-red-800/50 rounded-xl p-4">
          ...
          <div className="text-xs text-red-200/80 space-y-1.5">
            <p className="text-sm font-semibold text-red-300">This cannot be undone</p>
            <p>
              <strong>{expected}</strong> (VMID {vm.vmid}) will be permanently destroyed, including its disks
              and <strong>all of its backups</strong>. If the VM is running it will be force-stopped first.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            disabled={deleting}
            autoFocus
            className="w-full bg-gray-800 border border-gray-700/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/20 transition-all"
          />
        </div>

        {error && <p className="text-xs text-red-400 bg-red-900/20 rounded-lg p-2.5">{error}</p>}
        {deleting && (
          <p className="text-xs text-yellow-300/80 bg-yellow-900/20 rounded-lg p-2.5">
            Deleting VM and backups — this can take a few minutes. Keep this page open.
          </p>
        )}

        <div className="flex items-center justify-end gap-3">
          <button onClick={onClose} disabled={deleting} className="text-xs px-4 py-2 text-gray-400 hover:text-white transition-colors disabled:opacity-40">
            Cancel
          </button>
          <button
            onClick={doDelete}
            disabled={!confirmed || deleting}
            className="text-xs px-5 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-40 text-white rounded-lg font-medium transition-colors inline-flex items-center gap-2"
          >
            {deleting && (
              <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
            )}
...
function SnapshotsSection({ node, vmid }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="aaris-display text-sm text-gray-100">Snapshots</h2>
      ...
      {browseBackup && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800">
            <div>
              <h3 className="aaris-display text-sm text-gray-100 flex items-center gap-2">
