...
export default function PVEHostsPage() {
  ...
  const remove = async (h) => {
    if (!(await confirm({ title: 'Delete PVE host', message: `Delete "${h.name}"? This removes it from Homelabrrr.`, confirmLabel: 'Delete', danger: true }))) return;
    try {
      await api.delete(`/admin/pve-hosts/${h.id}`);
      load();
    } catch (e) {
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => openEdit(h)} aria-label={`Edit ${h.name}`} className="text-gray-500 hover:text-white p-1.5 rounded-lg hover:bg-gray-800 transition-colors">
                      ...
                      <button onClick={() => remove(h)} aria-label={`Delete ${h.name}`} className="text-gray-500 hover:text-red-400 p-1.5 rounded-lg hover:bg-red-500/10 transition-colors">
                              ...
                              <div key={n.node} className="bg-gray-800/50 border border-gray-700/30 rounded-xl px-4 py-3 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-6 text-xs">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div className="w-20 bg-gray-700/50 rounded-full h-1.5 overflow-hidden">
                                      <div className={`h-full rounded-full transition-all ${memPct > 80 ? 'bg-red-500' : memPct > 50 ? 'bg-yellow-500' : 'bg-gray-500'}`} style={{ width: `${memPct}%` }} />
                                    </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={e => { if (e.target === e.currentTarget) setShowForm(false); }}>
          <form ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="pve-modal-title" onSubmit={save} className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
              <h2 id="pve-modal-title" className="text-white font-semibold">{editId ? 'Edit Host' : 'Add PVE Host'}</h2>
            ...
            <div className="p-5 space-y-4">
              <div>
                <label htmlFor="pve-name" className="block text-xs text-gray-400 mb-1.5 font-medium">Name</label>
                <input
                  id="pve-name" type="text" required value={form.name}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <label htmlFor="pve-host" className="block text-xs text-gray-400 mb-1.5 font-medium">Host / IP</label>
                  <input
                    id="pve-host" type="text" required value={form.host}
                    onChange={e => setForm(f => ({ ...f, host: e.target.value }))}
                    className={inputCls} placeholder="192.168.1.10"
                  />
                </div>
                <div>
                  <label htmlFor="pve-port" className="block text-xs text-gray-400 mb-1.5 font-medium">Port</label>
                  <input
                    id="pve-port" type="number" value={form.port}
              ...
              <div>
                <label htmlFor="pve-token-id" className="block text-xs text-gray-400 mb-1.5 font-medium">API Token ID</label>
                <input
                  id="pve-token-id" type="text" required value={form.tokenId}
              ...
              <div>
                <label htmlFor="pve-token-secret" className="block text-xs text-gray-400 mb-1.5 font-medium">
                  API Token Secret {editId && <span className="text-gray-600">(leave empty to keep current)</span>}
                </label>
                <input
                  id="pve-token-secret" type="password" required={!editId} value={form.tokenSecret}
