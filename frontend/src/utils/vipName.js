...
//
// This is a verbatim copy of backend/src/utils/vipName.js so the name previewed
// here matches what the backend persists on FortiGate. Keep the two in sync —
// including the hash — or previews will drift. (Pinned hash values are asserted
// in backend/src/utils/vipName.test.js.)
...
// FortiOS policy names share the same 35-char limit, so the rule name itself
// must leave room for that prefix — otherwise the failure just moves from the
// VIP step to the policy step.
...
// across Node and browser runtimes, so this matches the backend copy exactly.
