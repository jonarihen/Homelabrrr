...
## 2026-09-18 — Malformed operation links no longer crash with a server error

- **Opening an operation reconcile, resolve, or cleanup link with a non-numeric id answered `500`.** The operations routes passed the raw parameter into an integer column, which PostgreSQL rejects — the same class as the website, template, and workflow routes fixed just before
- **Non-numeric operation ids now resolve to "not found".** All six endpoints answer their existing `404`. A regression test covers every route
...
- **The whole edit is now all-or-nothing.** Every field is checked before anything is written, and the name, description, permissions, and quotas are then applied together in a single database transaction. A rejected save leaves the role exactly as it was, and a database failure partway through rolls the rest of it back rather than leaving a role half-updated. Successful saves are unchanged — same fields, same audit entry
- **Creating a role is atomic too**, for the same reason: a new role and its permission set now commit together instead of as two separate writes
...
- **Account recovery no longer depends on one fragile second factor.** Users can register multiple named passkeys, generate hashed single-use recovery codes, inspect active sessions, revoke an individual session, or sign out every other device. Forced 2FA enrollment remains able to reauthenticate even if the confirmation window expires
- **SSH and SFTP access is constrained to the selected VM.** Non-admin destinations must resolve entirely to an address assigned to the VM or one of the user's VLANs; ports are validated, attempts are durably rate-limited, host keys remain pinned, and unusual administrator overrides are explicit and audited. SSH/SFTP failures no longer expose remote hosts, paths, usernames, or fingerprints, and the file browser now supports audited rename operations
- **Interrupted Proxmox work is recoverable instead of guessed failed.** Provisioning and migration rows retain their upstream task ID, phase, actor, and request ID. A restart moves unfinished work to **Admin → Operations**, where an operator can inspect the latest upstream result, manually verify the resource, mark the established final state, and remove only terminal portal tracking without deleting anything in Proxmox
- **Portal database backups are now online, encrypted, off-host, and restore-tested.** Each scheduled or manual run takes a SQLite-consistent snapshot, encrypts it with a separate backup key, copies it to the disaster-recovery destination, decrypts that copy into isolation, and verifies integrity/schema before reporting success. A restore CLI and rollback runbook are included, and destination failures remain visible in Operations
- **Operational health is visible and shutdown is bounded.** Separate liveness/readiness probes, Prometheus metrics, structured redacted JSON logs, request IDs, actor correlation, and outcome-aware mutation auditing cover normal and denied work. SIGTERM stops new HTTP/background admission, closes consoles, drains schedulers and long-running jobs for a bounded period, checkpoints WAL, and leaves durable unfinished jobs for restart reconciliation
- **Database growth and secret rotation have runbooks rather than surprises.** Incremental retention removes only bounded batches of old terminal history, performs safe WAL maintenance without an online full vacuum, and reports database/WAL/reclaimable size. Stored secrets use versioned key IDs with a dry-run plan, one-transaction rotation, legacy-key rollback window, and tests proving a bad value rolls the whole rotation back
- **Host deletion and infrastructure errors now fail safely.** Removing a Proxmox host lists and blocks dependent resources, refuses to delete the final host, and can remove a clean unreachable host without requiring a successful upstream probe. VNC response capture is bounded/redacted, SSH/SFTP error taxonomies are stable, and broad audit events omit request bodies and credential-shaped data
- **Frontend authorization is enforced before protected pages load.** Direct URLs use the same permission map as navigation, refreshes apply live permission changes before page data is requested, and browser tests cover sign-in, delegated access, denial, and permission revocation. Account security and operations were split into focused components, while backend host, backup, maintenance, reconciliation, and SSH policy logic moved behind service boundaries
- **The supported runtime and delivery pipeline were modernized.** The project now targets Node 24, Express 5, Vite 8, current WebAuthn/noVNC dependencies, and refreshed Node/nginx container bases. CI runs backend lint/tests, frontend lint/unit/browser tests and production builds, plus both container builds; Dependabot tracks npm, Docker, and GitHub Actions updates
- **Existing installations upgrade in place.** Ordered transactional schema migrations adopt the legacy database without losing users, abort startup on corruption or unexpected DDL failures, and are idempotent on restart. The container corrects ownership of an existing `db_data` volume before dropping to the unprivileged Node user. The README now documents backup, restore, key rotation, retention, observability, shutdown, API-token scopes, module boundaries, and upgrade constraints

## 2026-08-04 — A published website is only "live" once it actually serves

- **A site that never serves is no longer reported as live.** Publishing marked the site live as soon as Caddy's admin API accepted the route. If the Caddy host's hand-written Caddyfile already had a block for the same hostname, that block always won — Caddy takes the first matching route and stops — so the portal's route was dead config from the moment it was created, and visitors got a 502 while every step in the stepper stayed green
- **Conflicting routes are now detected and named.** After pushing, the live route array is read back; a route that matches the same domain and is reached first puts the site into a new **conflict** state, naming the upstream it proxies to instead and pointing at the Caddyfile. That route is never deleted or rewritten — Homelabrrr only ever touches routes it created — so clearing the block and hitting **Retry** is the fix
- **Every publish ends with a real HTTPS request to the domain.** It goes straight to the Caddy host with the domain as SNI and `Host` (no public DNS, so hairpin NAT can't break it), and only a site that answers becomes live. The failure modes stay separate: a 502 is your own upstream being down, an untrusted certificate is Let's Encrypt still working, a refused connection is the Caddy host
- **Published sites are re-checked in the background**, so a site that breaks later drops to a warning on its own and returns to live when it recovers — no user has to report it first
- **Sites can no longer be published to a plaintext-only Caddy server.** Picking which Caddy http server to attach to accepted `:443` or `:80` with no preference; a `:80`-only server sorting first meant every site landed on the plaintext listener with no certificate and no HTTPS, and still reported success. `:443` now wins outright, the chosen server is named in the stepper, and a `:80`-only situation is reported instead of published
- **The self-heal loop is now conflict-aware and configurable.** Re-pushing a route dropped by a `caddy reload` re-runs the same conflict check, so a route can't be silently restored into a shadowed position. Its interval is the new `WEBSITE_RECONCILE_INTERVAL_MS` (default 5 min), and the README now documents that admin-API routes live only in Caddy's running config, what the reconcile loop does and doesn't cover, and Caddyfile snippet sync as the durable alternative
