...
test('mutation audit records success, denial, and failure without request bodies or secrets', () => {
  ...
  try {
    const result = spawnSync(process.execPath, ['--input-type=module', '-e', `
      ...
      const { auditMutations } = await import('./src/middleware/mutationAudit.js');
...
test('high-risk route sources declare stable mutation audit actions', async () => {
  ...
  const admin = await readFile(new URL('../routes/admin.js', import.meta.url), 'utf8');
  const ssh = await readFile(new URL('../routes/ssh.js', import.meta.url), 'utf8');
  const sftp = await readFile(new URL('../routes/sftp.js', import.meta.url), 'utf8');
