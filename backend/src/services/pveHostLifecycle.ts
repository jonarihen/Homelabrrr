...
async function tableExists(database: DbOrTx, name: string): Promise<boolean> {
  const result = await database.execute(sql`SELECT to_regclass(${`public.${name}`}) IS NOT NULL AS present`);
  return Boolean((result.rows[0] as { present: boolean }).present);
}

async function countWhere(database: DbOrTx, fragment: ReturnType<typeof sql>): Promise<number> {
  const result = await database.execute(fragment);
  return Number((result.rows[0] as { count: number | string }).count);
}

export async function pveHostDependencies(hostId: number | string) {
  return pveHostDependenciesTx(db, hostId);
}

async function pveHostDependenciesTx(database: DbOrTx, hostId: number | string) {
  ...
  if (await tableExists(database, 'vm_migrations')) {
    const c = await countWhere(
      database,
...
export async function deletePveHost(hostId: number | string) {
  return db.transaction(async (tx) => {
    // Lock every host row before counting: two concurrent deletes must not
    // both observe N >= 2 and remove the last host between them. The count
    // and the delete below run in the same transaction on the locked rows.
    const rows = await tx.select({ id: pveHosts.id }).from(pveHosts).for('update');
    if (rows.length <= 1) {
    ...
    const report = await pveHostDependenciesTx(tx, hostId);
    if (report.total > 0) {
