...
test('flags the exact bug shape from issue #71', () => {
  ...
  const found = scanForLegacyPermissionColumnReads('src/routes/admin.js', src);
  assert.equal(found.length, 1);
  assert.equal(found[0].line, 2);
  assert.equal(found[0].file, 'src/routes/admin.js');
});
...
test('flags other spellings: multiple columns, lowercase sql, aliased table', () => {
  ...
  for (const line of cases) {
    assert.equal(
      scanForLegacyPermissionColumnReads('src/routes/x.js', line).length, 1,
      `should flag: ${line}`,
    );
  }
});
...
test('does not flag unrelated queries or prose about the pattern', () => {
  ...
  assert.deepEqual(scanForLegacyPermissionColumnReads('src/routes/x.js', src), []);
});

test('handles empty / missing contents without throwing', () => {
  assert.deepEqual(scanForLegacyPermissionColumnReads('a.js', ''), []);
  assert.deepEqual(scanForLegacyPermissionColumnReads('a.js', null), []);
});

// ─── the allowlist ──────────────────────────────────────────────────────────

const PROVISION_LINE = {
  file: 'src/routes/provision.js',
  line: 134,
...
test('the same query in any other file is NOT sanctioned', () => {
  const copied = { ...PROVISION_LINE, file: 'src/routes/admin.js' };
...
test('the allowlist is still live — the sanctioned read is the only match', () => {
  ...
  assert.match(findings[0].file, /routes\/provision\.js$/);
});

test('the sanctioned provision.js read really does fold the role back in', () => {
  const src = fs.readFileSync(path.join(SRC_ROOT, 'routes', 'provision.js'), 'utf8');
