...
test('versioned keyring decrypts legacy ciphertext, rotates it, and permits old-key retirement', () => {
  const oldCiphertext = runModule(
    "import { encryptSecret } from './src/utils/secrets.ts'; process.stdout.write(encryptSecret('mixed-key-secret'));",
  ...
  const rotated = JSON.parse(runModule(
    "import { decryptSecret, encryptSecret, secretNeedsMigration } from './src/utils/secrets.ts'; const old=process.env.VALUE; process.stdout.write(JSON.stringify({plain:decryptSecret(old),needs:secretNeedsMigration(old),next:encryptSecret(decryptSecret(old))}));",
    {
  ...
  const retiredPlaintext = runModule(
    "import { decryptSecret } from './src/utils/secrets.ts'; process.stdout.write(decryptSecret(process.env.VALUE));",
...
test('missing legacy keys fail closed without exposing plaintext', () => {
  const oldCiphertext = runModule(
    "import { encryptSecret } from './src/utils/secrets.ts'; process.stdout.write(encryptSecret('do-not-print-me'));",
    { SECRET_ENCRYPTION_KEY_ID: 'old', SECRET_ENCRYPTION_KEY: OLD_KEY, SECRET_ENCRYPTION_PREVIOUS_KEYS: '' },
  );
  const message = runModule(
    "import { decryptSecret } from './src/utils/secrets.ts'; try { decryptSecret(process.env.VALUE); } catch (error) { process.stdout.write(error.message); }",
...
test('database rotation rolls back every earlier update when one record is undecryptable', () => {
  const oldCiphertext = runModule(
    "import { encryptSecret } from './src/utils/secrets.ts'; process.stdout.write(encryptSecret('transaction-secret'));",
    { SECRET_ENCRYPTION_KEY_ID: 'old', SECRET_ENCRYPTION_KEY: OLD_KEY, SECRET_ENCRYPTION_PREVIOUS_KEYS: '' },
  );
