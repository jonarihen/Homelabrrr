...
- **VM-level access** (`utils/vmAccess.js`): `userCanAccessVm` (assignment OR `see_all_vms` OR admin) for reads; `userOwnsVm` (strict assignment only, no `see_all_vms` bypass) gates destructive ops like deletion.
